...
* 8/28: My test today turned out to be with errors, actually. The issue was `Assignments` and `Quizzes/Tests` modules failed to import. After the third attempt with `I would like to replace my data` option selected, those modules were copied as expected. As I did not see any error messages, I had no idea about the source of this error. 
