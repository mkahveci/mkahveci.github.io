...
classDef c-link stroke:#333;

A[fa:fa-sitemap Start: Analyze Structure]:::c-start
B((fa:fa-compass HTF Daily/4H: Define Trend)):::c-process
C{fa:fa-filter Trend Aligns?}:::c-decision
D((fa:fa-search MTF 1H: Spot Setup)):::c-process
E{fa:fa-chart-line Setup Aligns?}:::c-decision
F((fa:fa-clock LFT 5-15M: Confirm Entry)):::c-process
G[fa:fa-arrow-circle-right Execute Trade]:::c-end
H[fa:fa-times Abandon / Wait]:::c-end
I((fa:fa-undo Back to HTF)):::c-process
...
E-- Yes -->F:::c-link
F-->G:::c-link'
caption='1. The Multi-Timeframe Analysis (MTF) Flow. This top-down hierarchy ensures the entry setup on lower timeframes always aligns with the dominant trend.'
%}
...
A["fa:fa-clipboard-check Start: Run Checklist"]:::c-start
B(("fa:fa-sitemap 1. HTF Define Regime")):::c-process
C(("fa:fa-chart-bar 2. Context Check: VWAP, VA, BB")):::c-process
D{"fa:fa-balance-scale Regime & Indicators Align?"}:::c-decision
E(("fa:fa-drafting-compass 3. Map Strategy (Trend/Range)")):::c-process
F{"fa:fa-check-double 4. Spot MTF Setup Aligns?"}:::c-decision
G{"fa:fa-bullseye 5. LFT Confirmation at Key Level?"}:::c-decision
H["fa:fa-rocket Execute Trade (To Risk Mgmt)"]:::c-end
I["fa:fa-pause-circle Abandon / Wait"]:::c-end
...
G -- "No" --> I
G -- "Yes" --> H'
caption='2. The Systematic Pre-Trade Checklist Flow. This workflow ensures all analytical steps are completed before execution.'
%}
...
classDef c-start fill:#E0F7FA, stroke:#00BCD4, stroke-width:3px, color:#212121, font-weight:bold;
classDef c-process fill:#E3F2FD, stroke:#2196F3, stroke-width:2px, color:#212121;
classDef c-calc fill:#FFF9C4, stroke:#FBC02D, stroke-width:2px, color:#212121;
classDef c-end fill:#F3E5F5, stroke:#9C27B0, stroke-width:3px, color:#212121, font-weight:bold;
classDef c-link stroke:#333;

A["fa:fa-lightbulb-o Start: High-Probability Setup Identified"]:::c-start
B(("fa:fa-line-chart 1. Get Current ATR Value <br> (e.g., 14-period)")):::c-process
C(("fa:fa-calculator 2. Calculate Stop-Loss Distance ($) <br> e.g., Stop-Loss = 2x ATR Value")):::c-calc
D(("fa:fa-calculator 3. Calculate Position Size <br> Size = Max Acct. Risk ($) / Stop-Loss Distance ($)")):::c-calc
E["fa:fa-check-circle End: Risk & Position Size Defined. <br> Ready for Entry."]:::c-end

A --> B
B --> C
C --> D
D --> E'
caption='3. The Dynamic Risk Architecture Flow. This process must be completed *after* a setup is found but *before* an entry is taken.'
%}
...
classDef c-start fill:#E0F7FA, stroke:#00BCD4, stroke-width:3px, color:#212121, font-weight:bold;
classDef c-process fill:#E3F2FD, stroke:#2196F3, stroke-width:2px, color:#212121;
classDef c-action fill:#E8F5E9, stroke:#4CAF50, stroke-width:2px, color:#212121, font-weight:bold;
classDef c-end fill:#F3E5F5, stroke:#9C27B0, stroke-width:3px, color:#212121, font-weight:bold;
classDef c-link stroke:#333;

A["fa:fa-play-circle Trade Executed (Full Position)"]:::c-start
B{"fa:fa-bullseye Price reaches Target 1 <br> (e.g., 1:1 R/R)"}:::c-process
C["fa:fa-money Action: <br> 1. Sell 1/3 Position <br> 2. Move Stop-Loss to Breakeven"]:::c-action
D{"fa:fa-bullseye Price reaches Target 2 <br> (e.g., 2:1 R/R)"}:::c-process
E["fa:fa-money Action: <br> 1. Sell 1/3 Position"]:::c-action
F["fa:fa-cogs Manage final 1/3 <br> with ATR Trailing Stop"]:::c-process
G["fa:fa-flag-checkered Final 1/3 Exits on Trailing Stop"]:::c-end
...
E --> F
F --> G'
caption='4. Example "Scaling Out" Trade Management Flow. This mechanical process helps secure profits and reduce emotional anxiety.'
%}
...
{% include figure.html
mermaid_code='graph TD
classDef c-start fill:#FFEBEE, stroke:#D32F2F, stroke-width:3px, color:#212121, font-weight:bold;
classDef c-process fill:#E3F2FD, stroke:#2196F3, stroke-width:2px, color:#212121;
classDef c-decision fill:#FFFDE7, stroke:#FFC107, stroke-width:2px, color:#21212S:21;
classDef c-action fill:#FCE4EC, stroke:#EC407A, stroke-width:2px, color:#212121, font-weight:bold;
classDef c-end fill:#E8F5E9, stroke:#4CAF50, stroke-width:3px, color:#212121, font-weight:bold;
classDef c-link stroke:#333;

A["fa:fa-thumbs-o-down Start: Trade Closes for a Loss"]:::c-start
B{"fa:fa-warning Recognize Emotional Signals? <br> (Anger, Frustration, Urge to Get Back)"}:::c-decision
C["fa:fa-ban Enforce Cooling-Off Period. <br> **STOP TRADING.**"]:::c-action
D["fa:fa-search Conduct Post-Mortem Analysis <br> (Process Error or Valid Loss?)"]:::c-process
E["fa:fa-pencil-square-o Journal the Trade, <br> Execution, and Emotions"]:::c-process
F["fa:fa-refresh Return to Trading <br> (Emotionally Neutral)"]:::c-end
G["fa:fa-check-circle-o Accept Loss as Cost of Business <br> (No Emotional Signal)"]:::c-end
...
D --> E
E --> F'
caption='5. Protocol for Conquering Emotional Interference. This workflow is a non-negotiable part of psychological discipline.'
%}
