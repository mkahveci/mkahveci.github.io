...
A["fa:fa-sitemap Multi-Timeframe Analysis (MTA)"]:::c-start
A --> B["fa:fa-clock Directional Timeframe (1-Hour Chart)"]:::c-process
B --> C{fa:fa-filter Analyze 200-period EMA}:::c-decision
C -- "Price > 200 EMA" --> D["fa:fa-arrow-up Set Bias: Longs Only"]:::c-process
C -- "Price < 200 EMA" --> E["fa:fa-arrow-down Set Bias: Shorts Only"]:::c-process
D & E --> F["fa:fa-mouse-pointer Execution Timeframe (5-Minute Chart)"]:::c-process
F --> G["fa:fa-tasks Monitor for Confluence Signals"]:::c-end
' caption='1. The Top-Down Multi-Timeframe Analysis (MTA) workflow, where the 1-Hour chart dictates the trading bias.' %}

{% include figure.html
image_url='/images/blog/ofd/01.png'
image_alt='Description of chart data.'
caption='2. Image of a 1-hour /MNQ chart showing the 200 EMA with "Longs Only" and "Shorts Only" zones.'
%}
...
classDef c-decision fill:#FFFDE7, stroke:#FFC107, stroke-width:2px, color:#212121;
classDef c-end fill:#F3E5F5, stroke:#9C27B0, stroke-width:3px, color:#212121, font-weight:bold;
A["fa:fa-play-circle Start: Pre-Trade Check"]:::c-start
A --> B{"fa:fa-clock Price < 1-Hour 200 EMA?"}:::c-decision
B -- "No (Bias Fails)" --> C["fa:fa-ban STOP. Shorts are not permitted."]:::c-end
B -- "Yes (Bias OK)" --> D{"fa:fa-chart-line MACD Cross < Signal? (5-Min)"}:::c-decision
D -- "No" --> E["fa:fa-pause Wait for signal"]:::c-process
E --> D
D -- "Yes (Trigger)" --> F{"fa:fa-tachometer-alt RSI < 50? (5-Min)"}:::c-decision
F -- "No (Filter Failed)" --> G["fa:fa-times-circle No Trade. Confluence failed."]:::c-end
F -- "Yes (Confirmation 1)" --> H{"fa:fa-check-double Stochastic < 50? (5-Min)"}:::c-decision
H -- "No (Filter Failed)" --> G
H -- "Yes (All Conditions Met)" --> I["fa:fa-arrow-circle-down ENTER SHORT TRADE"]:::c-process
I --> J["fa:fa-search Monitor for Exit"]:::c-process
J --> K{"fa:fa-chart-line MACD Cross > Signal Line?"}:::c-decision
K -- "No" --> J
K -- "Yes (Exit Signal)" --> L["fa:fa-stop-circle EXIT TRADE"]:::c-end
' caption='4. Flowchart of the complete Short Trade confluence model, from entry check to the MACD-based exit signal.' %}
...
A["fa:fa-play-circle Start: Pre-Trade Check"]:::c-start
A --> B{"fa:fa-clock Price > 1-Hour 200 EMA?"}:::c-decision
B -- "No" --> C["fa:fa-ban STOP. Longs are not permitted."]:::c-end
B -- "Yes (Bias OK)" --> D{"fa:fa-chart-line MACD Cross > Signal? (5-Min)"}:::c-decision
D -- "No" --> E["fa:fa-pause Wait for signal"]:::c-process
E --> D
D -- "Yes (Trigger)" --> F{"fa:fa-tachometer-alt RSI > 50? (5-Min)"}:::c-decision
F -- "No" --> G["fa:fa-times-circle No Trade. Filter failed."]:::c-end
F -- "Yes (Confirmation 1)" --> H{"fa:fa-check-double Stochastic > 50? (5-Min)"}:::c-decision
H -- "No" --> G
H -- "Yes (All Conditions Met)" --> I["fa:fa-arrow-circle-up ENTER LONG TRADE"]:::c-process
I --> J["fa:fa-search Monitor for Exit"]:::c-process
J --> K{"fa:fa-chart-line MACD Cross < Signal Line?"}:::c-decision
K -- "No" --> J
K -- "Yes (Exit Signal)" --> L["fa:fa-stop-circle EXIT TRADE"]:::c-end
' caption='3. Flowchart of the complete Long Trade confluence model, from entry check to the MACD-based exit signal.' %}
...
classDef c-decision fill:#FFFDE7, stroke:#FFC107, stroke-width:2px, color:#212121;
classDef c-end fill:#F3E5F5, stroke:#9C27B0, stroke-width:3px, color:#212121, font-weight:bold;
A["fa:fa-calculator Start: Get Trade Inputs"]:::c-start
A --> B["fa:fa-ruler-combined Calc Stop Distance (Points)<br/>(e.g., ATR Value x 2.0 Multiplier)"]:::c-process
B --> C["fa:fa-compass <b>Define Initial Risk (1R)</b><br/>1R = Stop Distance in Points"]:::c-process
C --> D{"fa:fa-balance-scale Choose R:R Multiple"}:::c-decision
D -- "1.5R Target" --> E["fa:fa-bullseye Calc Target = 1R x 1.5"]:::c-process
D -- "2.0R Target" --> F["fa:fa-bullseye Calc Target = 1R x 2.0"]:::c-process
E & F --> G["fa:fa-flag-checkered Set Final Profit Target Price"]:::c-end
' caption='6. The systematic process for setting a profit target based on a Risk-to-Reward (R) multiple.' %}
...
subgraph Inputs
direction LR
A["fa:fa-dollar-sign Account Equity"]:::c-start
B["fa:fa-percent Account Risk Pct"]:::c-start
C["fa:fa-wave-square ATR Value (Points)"]:::c-start
D["fa:fa-times ATR Multiplier"]:::c-start
E["fa:fa-gavel USD Value per Point"]:::c-start
end
subgraph Calculations
direction TB
F["fa:fa-calculator Calc 1: Max Risk (USD) = Equity x Risk Pct"]:::c-process
G["fa:fa-calculator Calc 2: Stop Distance (Points) = ATR x Multiplier"]:::c-process
H["fa:fa-calculator Calc 3: Risk per Contract (USD) = Stop Distance x USD Value"]:::c-process
I["fa:fa-calculator Calc 4: Contracts = Max Risk / Risk per Contract"]:::c-process
end
J["fa:fa-cubes Final Position Size (Rounded Down)"]:::c-end
A & B --> F
C & D --> G
G & E --> H
F & H --> I
I --> J
' caption='5. The systematic calculation process for determining position size based on the 1% risk rule and dynamic ATR.' %}
...
classDef c-decision fill:#FFFDE7, stroke:#FFC107, stroke-width:2px, color:#212121;
classDef c-end fill:#F3E5F5, stroke:#9C27B0, stroke-width:3px, color:#212121, font-weight:bold;
A["fa:fa-sun Bull Market Allocation<br/>(70% Options / 10% Futures / 20% Stocks)"]:::c-start
A --> B{"fa:fa-cloud-rain Market Enters Bear Cycle?<br/>(e.g., Price < 200-Day MA)"}:::c-decision
B -- "No" --> A
B -- "Yes" --> C["fa:fa-exclamation-triangle Begin Defensive Re-allocation"]:::c-process
C --> D["fa:fa-shield-alt 1. Reduce Options (Shift to Hedges)"]:::c-process
C --> E["fa:fa-random 2. Repurpose Futures (Shift to Hedges)"]:::c-process
C --> F["fa:fa-briefcase-medical 3. De-Risk Equity (Rotate to Defensive)"]:::c-process
C --> G["fa:fa-piggy-bank 4. Increase Cash / Equivalents"]:::c-process
D & E & F & G --> H["fa:fa-umbrella-beach Defensive Portfolio Allocation<br/>(Capital Preserved)"]:::c-end
' caption='7. The strategic portfolio shift from a Bull Market to a Defensive Bear Market allocation.' %}
